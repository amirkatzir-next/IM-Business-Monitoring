...
WITH
    ...
    -- FIXED: Changed 'external_dwh.im_quotes' to 'temp_im_quotes'
    im_quotes_deduped AS (SELECT * FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY quote_id ORDER BY creation_time DESC) AS rn FROM temp_im_quotes) WHERE rn = 1),
    ...
    policies_table_pre AS (
        SELECT DISTINCT
            ...
            pt.package_name,
            a.affiliate_name,
            -- FIXED: Changed date_diff to DATEDIFF for Redshift compatibility
            DATEDIFF(day, pol.bind_date::date, a.start_date::date) as days_between_bind_to_policy_start
            -- END: Added fields
        ...
        -- FIXED: Changed 'external_dwh.im_submissions' to 'temp_im_submissions'
        LEFT JOIN temp_im_submissions sub ON iq.offer_id = sub.first_offer_id
